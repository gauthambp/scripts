bash-4.2$ more /usr/local/tomcat/scripts/deploy-webapps.sh
#! /bin/sh
#
...
#       where webapp = am css ddtoem esd partner paymentGateway profile utilsvc
#
# DEPENDENCIES:
#
...
for APP in ${WEBAPPS}; do
   case "${APP}" in
        am | ddtoem | esd | partner | paymentGateway | profile | utilsvc )
        ...
        * )
        echo "***ERROR: Invalid webapp ${APP} entered. Re-enter webapp (am, css, ddtoem, esd, partner, paymentGateway, profile or utilsvc)"
        exit 1
        ;;
   esac
done
...
# Specify the known build location (this is on lxappdev)
BUILD_PATH="/usr/local/releases/java-webapps/${USERNAME}/${ENVIRONMENT}-builds"
...
for APP in ${WEBAPPS}; do
   case "${APP}" in
        am | css | ddtoem | esd | partner | paymentGateway | profile | utilsvc )
        #----------------------------#
        ...
        * )
        echo "***ERROR: Invalid webapps. Specify am, css, ddtoem, esd, partner, paymentGateway, profile or utilsvc."
        exit 1
        ;;
   esac
done
